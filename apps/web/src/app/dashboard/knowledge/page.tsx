...
export default function KnowledgePage() {
  ...
  // --- View chunks ---
  const [viewChunksTitle, setViewChunksTitle] = useState<string | null>(null);
  ...
  const viewChunksData = trpc.knowledge.getByTitle.useQuery(
    { title: viewChunksTitle! },
    { enabled: !!viewChunksTitle },
  );
  ...
  return (
    <div className="space-y-8">
              ...
              {filteredDocs.map((doc) => (
                ...
                >
                  ...
                  <div className="ml-4 shrink-0">
                    {doc.title && deleteConfirm === doc.title ? (
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent>
                          <DropdownMenuItem onClick={() => handleEdit(doc.title!)}>
                            {tc('edit')}
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => setDeleteConfirm(doc.title!)}>
                            {tc('delete')}
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => setViewChunksTitle(doc.title!)}>
                            {t('viewChunks')}
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
      ...
      {/* View chunks modal */}
      <Dialog
        open={!!viewChunksTitle}
        onClose={() => setViewChunksTitle(null)}
      >
        <DialogHeader>
          <DialogTitle>
            {t('viewChunksDialogTitle', { title: viewChunksTitle ?? '' })}
          </DialogTitle>
        </DialogHeader>
