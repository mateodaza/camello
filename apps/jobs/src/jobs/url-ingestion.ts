...
export async function runUrlIngestion() {
  ...
  log.info(`Claimed ${claimed.length} sync rows`);
  ...
  for (const row of claimed) {
    try {
      ...
      log.info(`Synced: ${row.source_url}`);
      succeeded++;
    } catch (err) {
      ...
      log.error(`Failed (attempt ${row.attempt_count}/${MAX_RETRIES}): ${row.source_url} — ${message}`);
      failed++;
    }
  }

  const summary = { processed: claimed.length, succeeded, failed };
  log.info("URL ingestion complete", summary);
  return summary;
}
