...
| NC-286 | Tooltips — contextual help for key UI concepts | Mar 15 | Extended `tooltip.tsx` (`'use client'`, `position` prop, `InfoTooltip` component). Added `tooltip` prop + `e.preventDefault()` guard to `section.tsx`. Placed 14 `InfoTooltip` instances across `agent/page.tsx` (7: Skills, Approvals, Performance, Instructions, Tone, Constraints sub-block), `module-settings.tsx` (Approval Mode — sibling pattern), `trust-graduation-card.tsx` (Trust Graduation), `knowledge/page.tsx` (3: score, gaps header, scope toggle), `settings/page.tsx` (3: Webhook URL, Verify Token, Phone Number ID). Added `tooltips` namespace (14 keys) + 2 `agent` keys to `en.json`+`es.json`. New `tooltip.test.tsx` (3 tests). Updated `section.test.tsx` + `agent-page.test.tsx` (+1 test each). Type-check + build pass. |
...
| NC-272 | Advisor snapshot + dashboard panel + session learning | Mar 14 | `0028_knowledge_docs_advisor_source.sql` migration. New `advisor-snapshot.ts` lib + `routes/advisor.ts` (snapshot + summarizeSession procedures with advisor-conv verification). Snapshot injected into `message-handler.ts` for advisor artifacts. `advisor-panel.tsx` dashboard component. `test-chat-panel.tsx` extended with `initialMessages`/`onMessagesChange` props. 5 test suites: advisor-snapshot.test.ts, advisor-context-injection.test.ts, advisor-summarize.test.ts, advisor-panel.test.tsx, rag-advisor.test.ts. `pnpm type-check` passes. |
| NC-271 | Real-time inbox via Supabase Realtime Broadcast | Mar 14 | `supabase-broadcast.ts` (lazy admin client). Broadcast wired in `whatsapp.ts` + `conversation.ts` replyAsOwner. Browser: `supabase-client.ts` singleton + `use-realtime-inbox.ts` hook. `conversations/page.tsx` reads `publicMetadata.camello_tenant_id`, invalidates list always; byId/messages(no-args)/activity when conversationId matches. 7 new tests across 4 files. `pnpm type-check` passes. |
| NC-270 | WhatsApp dead-letter retry cron | Mar 14 | Migration 0027 adds `retry_count` column. `servicePool` added to `@camello/db/client.ts` + exported from index. New `apps/api/src/lib/service-pool.ts` (re-export). New `apps/api/src/routes/internal.ts` (POST /webhook-retry with channel_type + processed_at guards, 403/404 guards). `apps/api/src/app.ts` mounts `/api/internal`. New `apps/jobs/src/jobs/whatsapp-retry.ts` (5-min sweep, atomic claim, per-row fetch). `worker.ts`: REQUIRED_ENV + 5th cron. 11 tests: `whatsapp-retry.test.ts` (6), `internal-routes.test.ts` (4), `worker.test.ts` updated (5→5 schedules). `pnpm type-check` + `pnpm build` pass. |
| — | Legal pages + Paddle verification | Mar 13 | Created `/terms`, `/privacy`, `/refund` pages under `(legal)` route group with shared layout. Based on competitor research (11x, Relevance AI, Chatbase, Artisan). Key clauses: no-training commitment, Paddle MoR reference, 12-month liability cap, Colombian governing law (Ley 1581), GDPR+CCPA rights, AI disclaimers. i18n keys (en+es). Footer links activated. Paddle verification submitted — **waiting for Paddle review (1-3 business days)**. |
...
| CAM-200 | Clerk production keys + legal pages + onboarding fix | Mar 13 | **Clerk prod:** Created production instance (cloned from dev), Google OAuth with prod credentials, 5 DNS CNAMEs on Cloudflare (gray cloud), Svix webhook registered, env vars swapped on Railway + Vercel. **Legal pages:** Created `(legal)` route group with shared layout (`layout.tsx`), Terms of Service (17 sections), Privacy Policy (12 sections, processor table), Refund Policy (7 sections → rewritten to 14-day window per Paddle feedback). Footer links wired. 6 i18n keys (en+es). **Paddle verification:** Submitted, provisional approval, 3 feedback items addressed (refund qualifiers removed, 14-day window added, legal name → "Camello"). Production API key + client-side token generated. Products/prices pending plan finalization. Waiting for final verification approval. **Onboarding fix:** `onboarding.complete` → stale `tenant.me` tRPC cache caused redirect loop back to `/onboarding`. Fixed by adding `await utils.tenant.me.invalidate()` before `router.push('/dashboard')` in both `Step5TestIt.tsx` and `page.tsx` (skip setup). Type-check passes. |
| NC-262 | "Mark as Paid" action in Payments section | Mar 13 | Added `markPaymentPaid` tenantProcedure to `agent.ts` (preflight SELECT + PRECONDITION_FAILED guard). Updated `payments-section.tsx` with `trpc.agent.markPaymentPaid.useMutation`, actions column with `e.stopPropagation()`, invalidate + toast on success. 3 i18n keys (en+es). 2 tests in `mark-payment-paid.test.ts`. Type-check passes. |
| NC-260 | Two-way WhatsApp conversation sync | Mar 13 | Fixed `findActiveConversation` bug: `inArray(status, ['active','escalated'])` replaces `eq(status, 'active')` so post-escalation inbound messages continue the same conversation. Created `whatsapp-roundtrip.test.ts` (5 tests: new-conv flow, reuse flow, replyAsOwner WA delivery, escalated-conv behavioral, pg-proxy SQL regression). Modified `message-handler.ts` (3 lines). 427 tests pass. |
...
| NC-269 | PR audit + hardening (NC-257–268 branch) | Mar 13 | 4-agent PR review (code, errors, tests, comments). Critical fixes: `pool.query()` for both SECURITY DEFINER RPCs in `adapters/whatsapp.ts` (tsup `sql` tag bundling bug); `verifyWhatsapp` now throws `NOT_FOUND` instead of silently succeeding on 0-row update; `queueUrl` catch in `ChatOnboarding.tsx` now surfaces error via `setKnowledgeError`. Stale Trigger.dev comments replaced. `onboarding-advisor.test.ts` rewritten with correct single-transaction mock. 5 new tests (malformed JSON 400, empty tenant list 403, DB fail 500, verifyWhatsapp NOT_FOUND, markPaymentPaid NOT_FOUND). `text-error` → `text-sunset`, 2 new i18n keys (en+es). Type-check passes. |
...
- **NC-260** — 2026-03-13 — `7bfc8db` — Session: 20260313-151622-camello
- **NC-262** — 2026-03-13 — `9c26d07` — Session: 20260313-151622-camello
- **NC-263** — 2026-03-13 — `73ea706` — Session: 20260313-151622-camello
- **NC-264** — 2026-03-13 — `04d46fc` — Session: 20260313-151622-camello
- **NC-265** — 2026-03-13 — `20468f4` — Session: 20260313-151622-camello
- **NC-266** — 2026-03-13 — `f1f52d9` — Session: 20260313-151622-camello
- **NC-267** — 2026-03-13 — `2b40605` — Session: 20260313-151622-camello
- **NC-268** — 2026-03-13 — `176c2cd` — Session: 20260313-151622-camello
- **NC-270** — 2026-03-14 — `7dfd0ae` — Session: 20260314-011704-camello
...
- **NC-286** — 2026-03-15 — `a4f0992` — Session: 20260315-133858-camello
