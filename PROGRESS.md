...
| NC-268 | Advisor archetype + auto-create in onboarding | Mar 13 | Migration `0026_advisor_artifact_type.sql` adds 'advisor' to artifacts CHECK constraint. `packages/ai/src/archetypes/advisor.ts` self-registers with en/es prompts, `ragBias: {docTypes:['upload','url','api'], boost:1.5}`, `moduleSlugs:[]`, `icon:'BrainCircuit'`, `color:'gold'`. `onboarding.complete()` extended to idempotently create advisor artifact post-completion (3 sequential DB calls). `ArtifactType` union updated in `shared/types/index.ts`. Artifacts page shows gold-accented `AdvisorCard` (data-testid="advisor-card") with "Chat with Advisor" CTA. 2 i18n keys (en+es). 2 API tests + 1 web test. Type-check passes. |
| NC-267 | Onboarding polish: skip/resume + mobile + i18n | Mar 13 | Added `stageToStep` + `handleFinishLater` to `ChatOnboarding.tsx`; "I'll finish this later" button on all input stages (ask_description, confirm_agent, collecting_knowledge, ask_channel + sub-states); sticky `bottom-0 bg-sand py-2` wrappers on input divs; `animate-fade-in` on `ChatBubble` + avatar hidden below 360px; `@keyframes fade-in` + `--animate-fade-in` in `globals.css`; `onboarding-resume-banner` on `dashboard/page.tsx` with `onboarding.getStatus` gate; 3 i18n keys en+es (`finishLater`, `resumeSetupBanner`, `resumeSetupCta`); hoisted router spies + Test 7 in `chat-onboarding.test.tsx`; `mockGetStatus` + Tests 6–7 in `knowledge-nudge.test.tsx`. |
| NC-266 | Knowledge gap → teach prompt: inline answer flow | Mar 13 | Added `expandedGapIntent`/`gapAnswers`/`answeredGaps` state + `gapTeachIngest` mutation + `handleGapTeach`/`handleGapSave` to `knowledge/page.tsx`. Gap cards now show Teach button (outline), inline textarea when expanded (single open at a time), and Answered badge + opacity-50 on success. 6 i18n keys en+es. 2 tests added to `knowledge-page.test.tsx`. |
| NC-265 | "Teach more" inline input on Knowledge page | Mar 13 | Added `teachText`/`teachError` state + `teachIngest` mutation + `handleTeachSubmit` + UI block to `knowledge/page.tsx`. 4 i18n keys en+es. `mutationMocks` map + 2 new tests in `knowledge-page.test.tsx`. Type-check passes. |
| NC-264 | Knowledge sufficiency score + dashboard nudge | Mar 13 | Added `sufficiencyScore` tenantProcedure to `knowledge.ts` (doc count + synced URL + 30-day gap count → score formula + signals array). Updated `knowledge-banner.tsx` (gold styling, score + topSignal props). Updated `dashboard/page.tsx` (score<60 gate, raw-length artifact check). Added score widget to `knowledge/page.tsx` header (N/100 + label + Excellent/Good/Needs improvement bucket). 6 i18n keys en+es. Created `knowledge-sufficiency.test.ts` (5 tests). Updated `knowledge-nudge.test.tsx` (5 tests, sufficiencyScore mock). Gap query fix (iter 3): switched from `isNull(interactionLogs.resolutionType)` to counting `ownerNotifications` with `type='knowledge_gap'`. Cache fix (iter 4): added `utils.knowledge.sufficiencyScore.invalidate()` to `ingest` and `deleteByTitle` `onSuccess` handlers in `knowledge/page.tsx` so score widget refreshes after add/delete. |
